...
def main_worker():
    ...
    if args.local_rank == 0:
        log_dir = os.path.join(args.main_path, 'log', args.experiment + args.date)
    ...
    # ==========================model and writer===============================
    writer_path = os.path.join(args.main_path, 'tb-log', args.model_name, args.date)
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            writer.add_scalar('loss1',loss1.item(), global_step)
            writer.add_scalar('loss2',loss2.item(), global_step)
            writer.add_scalar('loss3',loss3.item(), global_step)
            loss_data['dice']['total_loss'] += loss.item()
        ...
        end_epoch = time.time()
        if args.local_rank == 0:
            loss_data['dice']['total_loss'] /= loss_data['iter']
            loss_data['dice']['loss1'] /= loss_data['iter']
            loss_data['dice']['loss2'] /= loss_data['iter']
            loss_data['dice']['loss3'] /= loss_data['iter']
            logging.info('mean_Epoch: {} loss: {:.5f} || 1:{:.4f} | 2:{:.4f} | 3:{:.4f} ||'
                         .format(epoch, loss_data['dice']['total_loss'], loss_data['dice']['loss1'],
                                 loss_data['dice']['loss2'], loss_data['dice']['loss3']))
            writer.add_scalars('final_loss', loss_data['dice'], epoch)

        if args.local_rank == 0:
            if loss_data['dice']['total_loss'] < bestloss:
                ...
                torch.save({
                    ...
                    file_name)
                bestloss = loss_data['dice']['total_loss']
